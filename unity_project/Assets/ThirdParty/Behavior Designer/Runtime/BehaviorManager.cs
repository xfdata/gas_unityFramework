...
namespace BehaviorDesigner.Runtime
{
    ...
    public class BehaviorManager : MonoBehaviour
    {
        ...
        private System.Object lockObject = new();
...
#if BD_TRIAL || BD_DISTRIBUTION || BD_EDUCATION
        private Texture2D watermark = null;
        private Rect watermarkRect;
        private GUIStyle trialGUIStyle;
#endif
...
#if BD_TRIAL || BD_DISTRIBUTION || BD_EDUCATION
            trialGUIStyle = new GUIStyle();
            trialGUIStyle.alignment = TextAnchor.MiddleCenter;
            trialGUIStyle.normal.textColor = Color.white;
            trialGUIStyle.fontSize = 15;
#endif
        }
            ...
            if (task is BehaviorReference) {
                ...
                if (behaviorSource != null) {
                    ...
                    for (int i = 0; i < behaviorSource.Length; ++i) {
                        ...
                        lock (lockObject) {
                            behaviorSource[i].CheckForSerialization(true, loadedBehaviorSource, true);
                        }
                        var externalRootTask = loadedBehaviorSource.RootTask;
                        externalRootTask.FriendlyName = task.FriendlyName;
                    ...
                    if (typeof(SharedVariable).IsAssignableFrom((fieldType = fields[i].FieldType.GetElementType())) ||
                        ...
                        if (list != null) {
                            for (int j = 0; j < list.Count; ++j) {
                                var overrideSharedVariable = OverrideSharedVariable(behaviorTree, data, fieldType, list[j] as SharedVariable);
                                if (overrideSharedVariable != null) {
                ...
                if (TaskUtility.HasAttribute(fields[i], typeof(InspectTaskAttribute))) {
                    if (typeof(IList).IsAssignableFrom(fields[i].FieldType)) {
                        ...
                        if (typeof(Task).IsAssignableFrom((fieldType = fields[i].FieldType.GetElementType()))) {
                            ...
                            if (list != null) {
                                for (int j = 0; j < list.Count; ++j) {
                                    OverrideFields(behaviorTree, data, list[j] as Task);
                                }
                            }
                        }
        ...
        private void Tick(BehaviorTree behaviorTree)
        {
            ...
            for (int i = 0; i < initialStackCount; ++i) {
                ...
                if (i < behaviorTree.interruptionTaskStatus.Count) {
                    ...
                    status = behaviorTree.interruptionTaskStatus[i];
                    int interruptedTask;
...
#if BD_TRIAL
            GUILayout.Label("Behavior Designer Trial Version");
#elif BD_DISTRIBUTION
            GUILayout.Label("Behavior Designer Distribution Version", trialGUIStyle);
#else
            GUILayout.Label("Behavior Designer Education Version", trialGUIStyle);
#endif
            GUILayout.FlexibleSpace();

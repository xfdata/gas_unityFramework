...
//by taecg
Shader "ONEMT/Effect/CommonEffectUI (SoftMaskable)"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass 
		{
			...
			// #pragma shader_feature_local _ _DEPTHDECAL_ON
			...
			// CBUFFER_START(UnityPerMaterial)
				float4 _MainTex_ST,_NoiseMap_ST,_HightMap_ST;
    ...
    //             	half DepthZ = LinearEyeDepth(Depth,_ZBufferParams);
	   //
				...
				// 	float depth = depthScene + i.positionVS.z;
				// 	c.a *= saturate(depth/_scale.w);
				//
				// 	clip(saturate(depth/_scale.w) - _Clip);
				// 	// return saturate(depth /10);
				// #endif
				...
				float softmask_alpha = SoftMask(i.positionCS, i.worldPosition, c.a); // Add for soft mask
				
				c.a *= softmask_alpha; // Add for soft mask
				...
				c.rgb = lerp(c.rgb * c.a, c.rgb, step(0.99, softmask_alpha));
